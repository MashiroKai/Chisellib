...
/**
  ...
  */
object MemFifo extends App {
  (new chisel3.stage.ChiselStage).emitVerilog(new MemFifo(UInt(8.W),12), Array("--target-dir","generated"))
}
...
class MemFifo[T <: Data](gen: T, depth: Int) extends Fifo(gen: T, depth: Int) {
  ...
  io.rd.bits :=  Mux(stateReg === valid, data, shadowReg)
  io.wr.ready := !fullReg
  io.rd.valid := stateReg === valid || stateReg === full
}
